...
function execution_tests.run(compile_file, backend, _ENV, only_compile)
   ...
   describe("Function calls /", function()
    ...
    describe("math.modf builtin", function()
        ...
        -- Note: In Lua, modf will return a float instead of an integer for NaN or if the float 
        -- does not fit in the int. Since Pallene enforces the declared types, we must return an
        -- integer. NaN only exists in floating point, so this is not possible to have in an int.
        it("Note: NaN behavior doesn't fully match Lua", function()
            run_test([[
                local integral, fractional = test.math_modf(0.0 / 0.0)
